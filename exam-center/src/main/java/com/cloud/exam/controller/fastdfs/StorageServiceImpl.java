...
public class StorageServiceImpl implements StorageService {
    ...
    @Autowired
    public StorageServiceImpl(@Value("${file.upload.dir}") String location) {
    ...
    public String uploadFileByMappedByteBuffer(MultipartFileParam param) throws IOException {
        ...
        if (isOk) {
            boolean flag = renameFile(tmpFile, fileName);
//            System.out.println(param.getId()+" upload complete !!" + flag + " name=" + fileName);
            logger.info(param.getId()+" upload 文件上传成功: [" + flag + "] name=" + fileName);
        }
    ...
    private  boolean checkAndSetUploadProgress(MultipartFileParam param, String uploadDirPath) throws IOException {
        ...
        //把该分段标记为 true 表示完成
        System.out.println(fileName +" set part : [" + param.getChunk() + "] complete");
        accessConfFile.setLength(param.getChunks());
        accessConfFile.seek(param.getChunk());
        accessConfFile.write(Byte.MAX_VALUE);

        //TODO 如何判断全部分片上传成功!! completeList 检查是否全部完成,如果数组里是否全部都是(全部分片都成功上传)
        ...
        if (isComplete == Byte.MAX_VALUE) {
            System.out.println(" ["+param.getName()+"]  线程["+param.getChunk()+"] 所有分先上传完成!!!");
