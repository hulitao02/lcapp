...
import com.cloud.gateway.config.RSAHelper;
...
//        String url = licence_url;
//        String message = adaptor.doGet(url, headMap, Collections.emptyMap());
//        log.info("调用授权返回message结果：" + message);
//        if (StringUtils.isNotEmpty(message)) {
//            JSONObject esObject = JSON.parseObject(message);
//            String messResult = esObject.get("message").toString();
//            byte[] byteKey = Base64.decodeBase64(key);
//            RSAHelper rsaHelper = new RSAHelper(byteKey);
//            RSAHelper.keySpecFromByte(byteKey, true);
//            byte[] data = Hex.decodeHex(messResult);
//            byte[] bytes = rsaHelper.decryptByPubKey(data);
//            String r = new String(bytes);
//            log.info("解码后结果：" + r);
//            JSONObject rObject = JSON.parseObject(r);
//            String status = rObject.get("status").toString();
//            // MISMATCH, 秘钥不匹配
//            // NORMAL, 正常授权
//            // EXPIRE, 授权已过期
//            // UNKNOWN_MODULE, 未知的模块
//            // ERROR; 验证授权出错
//            if (!status.equals("NORMAL")) {
//                log.warn("授权返回状态码status:" + status);
//                return null;
//            }
//        } else {
//            log.warn("授权返回message为空！");
//            return null;
//        }
...
//		parameters.put("username", username);
    ...
    private void saveLoginLog(String username, String remark) {
        ...
        CompletableFuture.runAsync(() -> {
            try {
                Log log = Log.builder().username(username).module(LogModule.LOGIN).remark(remark).flag(true).createTime(new Date())
                        .build();
                logClient.save(log);
            } catch (Exception e) {
                // do nothing
