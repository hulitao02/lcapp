function getMenuTree() {
    ...
    $.ajax({
        type : 'get',
        url : domainName + '/api-p/dataSource/getDataSourceInDataCollectionTree',
...
function initMenuDatas(id){
    $.ajax({
        type : 'get',
        url : domainName + '/api-p/TaskContent/getTaskContent/getTaskContentById?id=' + id,
        success : function(data) {
            var treeObj = $.fn.zTree.getZTreeObj("treeDemo");
            var data = eval("("+data.param+")");
            ...
            for(var i=0; i<length; i++){//选中节点
                var node = treeObj.getNodeByParam("name", data[i].dataTableName, null);
